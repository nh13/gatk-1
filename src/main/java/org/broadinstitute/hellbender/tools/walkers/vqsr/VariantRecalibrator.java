...
public class VariantRecalibrator extends VariantWalker {
    ...
    @Argument(fullName="rscriptFile", shortName="rscriptFile", doc="The output rscript file generated by the VQSR to aid in visualization of the input data and learned model", optional=true)
    ...
    public void onTraversalStart() {
        ...
        if( !dataManager.checkHasTrainingSet() ) {
            throw new UserException.CommandLineException(
                    "No training set found! Please provide sets of known polymorphic loci marked with the training=true ROD binding tag. For example, -resource:hapmap,VCF,known=false,training=true,truth=true,prior=12.0 hapmapFile.vcf" );
        }
        if( !dataManager.checkHasTruthSet() ) {
            throw new UserException.CommandLineException(
                    "No truth set found! Please provide sets of known polymorphic loci marked with the truth=true ROD binding tag. For example, -resource:hapmap,VCF,known=false,training=true,truth=true,prior=12.0 hapmapFile.vcf" );
        }
        ...
        if (hasReference()) {
            ...
            actualLines = VcfUtils.updateHeaderContigLines(hInfo, refFile, sequenceDictionary, true);
        }
        else {
            sequenceDictionary = getHeaderForVariants().getSequenceDictionary();
            if (null != sequenceDictionary) {
                actualLines = VcfUtils.updateHeaderContigLines(hInfo, null, sequenceDictionary, true);
            }
            else {
                actualLines = hInfo;
            }
        ...
        recalWriter = GATKVariantContextUtils.createVCFWriter(new File(output), getBestAvailableSequenceDictionary(), true);
        recalWriter.writeHeader( new VCFHeader(actualLines) );
    ...
    public void apply(VariantContext vc, ReadsContext readsContext, ReferenceContext ref, FeatureContext featureContext) {
        ...
        // TODO: this variant came from the drivingvariants input; do we still need to query it again for overlapping ?
        reduceSum.addAll( addOverlappingVariants(Collections.singletonList(getDrivingVariantsFeatureInput()), true, featureContext) );
        if( !aggregate.isEmpty()) {
            reduceSum.addAll( addOverlappingVariants(aggregate, false, featureContext) );
        }
    }

    /**
     * Using the RefMetaDataTracker find overlapping variants and pull out the necessary information to create the VariantDatum
     * @param featureInputs the input sources to search within
     ...
     */
    private List<VariantDatum> addOverlappingVariants(
            final List<FeatureInput<VariantContext>> featureInputs,
            final boolean isInput,
            final FeatureContext context ) {
        if( featureInputs == null ) { throw new IllegalArgumentException("featureInputs cannot be null."); }
        if( context == null ) { throw new IllegalArgumentException("context cannot be null."); }

        final ExpandingArrayList<VariantDatum> variants = new ExpandingArrayList<>();

        for( final VariantContext vc : context.getValues(featureInputs, context.getInterval().getStart()) ) {
            if( vc != null && ( IGNORE_ALL_FILTERS || vc.isNotFiltered() || ignoreInputFilterSet.containsAll(vc.getFilters()) ) ) {
                if( VariantDataManager.checkVariationClass( vc, VRAC.MODE ) && !VRAC.useASannotations) {
                    addDatum(variants, isInput, context, vc, null, null);
                }
                    ...
                    for(final Allele allele : vc.getAlternateAlleles()) {
                        ...
                        if (VariantDataManager.checkVariationClass(vc, allele, VRAC.MODE))
                            addDatum(variants, isInput, context, vc, vc.getReference(), allele);
                    }
                }
            }
        }
    ...
    private void addDatum(
        ...
        dataManager.decodeAnnotations(datum, vc, true); //BUGBUG: when run with HierarchicalMicroScheduler this is

        // non-deterministic because order of calls depends on load of machine
        datum.loc = (isInput ? new SimpleInterval(vc.getContig(), vc.getStart(), vc.getEnd()) : null);
        ...
        // the prior and training status appropriately
        dataManager.parseTrainingSets(featureContext, featureContext.getInterval(), vc, datum, TRUST_ALL_POLYMORPHIC);
